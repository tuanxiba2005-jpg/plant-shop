...
function applyFilters(status, startDate, endDate) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const activeBtn = document.querySelector(`.filter-btn[data-status="${params.status}"]`);
    if (activeBtn) activeBtn.classList.add('active');

    // Date filter listener
    const startDateInput = document.getElementById('adminFilterStartDate');
    const endDateInput = document.getElementById('adminFilterEndDate');
    
    const handleDateChange = () => {
        const start = startDateInput?.value;
        const end = endDateInput?.value;
        if (start && end) {
            applyFilters(params.status, start, end);
        }
    };
...
document.querySelectorAll('.filter-btn').forEach(btn => {
    btn.addEventListener('click', function () {
        ...
        const params = getFilterParams();
        applyFilters(status, params.startDate, params.endDate);
    });
});

window.clearAdminDateFilter = function() {
    const params = getFilterParams();
    applyFilters(params.status, '', '');
};
    ...
    try {
        ...
        if (data.success && data.order) {
            ...
            if (req.items && req.items.length > 0) {
                ...
                // Backward compatibility if no items specified (full return)
...
window.viewOrderDetail = async function(orderId) {
    ...
    try {
        // Staff route works exactly the same if we use /staff or /admin. We need to know which context we're in.
        const pathPrefix = window.location.pathname.startsWith('/staff') ? '/staff' : '/admin';
